 ...
 *      cb(false)      user ^C'd
 */
function promptYesNo(opts, cb) {
    assert.object(opts, 'opts');
    assert.string(opts.msg, 'opts.msg');
    assert.optionalString(opts.default, 'opts.default');

    process.stdout.write(opts.msg);
    ...
    function onData(ch) {
        ...
        case '\u0004':
            ...
            if (answer === '' && opts.default) {
                ...
                process.stdout.write('Please enter "y", "yes", "n" or "no".\n');
...
/**
 * Prompt on stdin for user input until an <Enter> and then `cb(<answer>)`.
 * This calls `cb(false)` if the user ^C'd.
 */
