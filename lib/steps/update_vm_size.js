...
/*
 * The values stored in SAPI objects (applications, services and instances)
 * have historically been somewhat lax in enforcement of data types for
 * property values.  If an operator has used "sapiadm" to update a numeric
 * property of a SAPI object, that value may have been stored as a stringified
 * version of the number.
 */
function loadAsNumber(obj, name) {
    ...
    if (typeof (v) === 'number') {
        if (isNaN(v) || !isFinite(v)) {
            return ({
                lan_present: true,
                lan_error: VError('property "%s" has invalid numeric value %j',
                    name, v)
            });
        }
    ...
    if (typeof (v) === 'string') {
        ...
        var conv = parseInt(v, 10);
        if (isNaN(conv) || !isFinite(conv) || String(conv) !== v) {
            return ({
                lan_present: true,
                lan_error: VError('property "%s" has a string value "%s" ' +
                    'that could not be converted to a number', name, v)
            });
        }
    ...
    return ({
        lan_present: true,
        lan_error: VError('property "%s" has a value of unexpected type ' +
            '"%s"; wanted a number', name, typeof (v))
    });
}
...
function determineChanges(currentValues, newValues, level) {
    ...
    Object.keys(newValues).forEach(function (name) {
        ...
        if (ALL_LEVEL_INCREASE_NUMBERS.indexOf(name) !== -1) {
            ...
            var lan = loadAsNumber(currentValues, name);

            if (lan.lan_error) {
                changes.chg_error = lan.lan_error;
                return;
            }
             ...
             */
            if (lan.lan_present && lan.lan_value >= updated) {
                mod_assert.ok(!lan.lan_error);
                return;
            }
            ...
            if (!lan.lan_present) {
                ...
                mod_assert.number(lan.lan_value);

                changes.chg_progress.push(sprintf('%s: %d -> %d', name,
                    lan.lan_value, updated));
            }
...
function updateSizeParametersVM(usp, vm, next) {
    ...
    if (chg.chg_error) {
        setImmediate(next, VError(chg.chg_error, 'updating size parameters ' +
            'for VM "%s" (uuid %s)', vm.alias, vm.uuid));
        return;
    }
...
function updateSizeParametersService(usp, next) {
    ...
    if (chg.chg_error) {
        setImmediate(next, VError(chg.chg_error, 'updating size parameters ' +
            'for SAPI service "%s" (uuid %s)', usp.usp_serviceName,
            usp.usp_serviceUuid));
        return;
    }
