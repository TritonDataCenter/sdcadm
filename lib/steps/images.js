 ...
 *   service.
 * - `args.channelArg`: Optional. The updates.tritondatacenter.com channel to
 *   use if querying updates.tritondatacenter.com. It defaults to the default
 *   configured channel (per `sdcadm channel`).
 * - `args.imgArg`: must be one of:
 *      - "latest": to find the latest available image on
 ...
 * - `args.channel`: Maybe. If updates.tritondatacenter.com was queried (i.e.
 *   the image was not found locally), then this will be set to
 *   `args.channelArg` or resolved to the default channel.
 *
...
function findSvcImg(args, cb) {
    ...
    vasync.pipeline({
        ...
        funcs: [
            ...
            // imgArg==<uuid> means look local IMGAPI or
            // updates.tritondatacenter.com.
            ...
            // imgArg==<version string>, try in updates.tritondatacenter.com if
            // needed
