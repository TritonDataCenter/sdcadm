...
RebootPlan.prototype.create = function create(opts, cb) {
    ...
    vasync.pipeline({funcs: [
        ...
        function savePlanIntoCnapi(_, next) {
            ...
            if (opts.name) {
                cnapiOpts.name = opts.name;
            }

            self.sdcadm.cnapi.post({
                path: '/reboot-plans'
            }, cnapiOpts, function cnapiCb(cnapiErr, cnapiRes) {
                if (cnapiErr) {
...
RebootPlan.prototype.getCurrentPlan = function getCurrentPlan(callback) {
    var self = this;
    var u = '/reboot-plans?state=running&include_reboots=true';
...
RebootPlan.prototype.getPlan = function getPlan(uuid, callback) {
    assert.string(uuid, 'uuid');
    assert.func(callback, 'callback');
    var self = this;
    var u = format('/reboot-plans/%s', uuid);
    self.sdcadm.cnapi.get(u, function (err, plan) {
        if (err) {
            callback(new errors.SDCClientError(err, 'cnapi'));
            return;
        }
        callback(null, plan);
    });
...
RebootPlan.prototype.run = function run(opts, callback) {
    ...
    self.getPlan(opts.uuid, function (err, plan) {
        if (err) {
            callback(err);
            return;
        }
        ...
        self.sdcadm.cnapi.put(planUri, planOpts, function (cnapiErr) {
            ...
            p('Plan execution has been %s',
                    (plan.state === 'created' ? 'started' : 'continued'));
...
RebootPlan.prototype.status = function status(uuid, callback) {
    var self = this;
    assert.string(uuid, 'uuid');
    self.getPlan(uuid, function (err, plan) {
        if (err) {
            callback(err);
            return;
        }
        ...
        self.getCoreServers(function (cSErr, cSUuids) {
            ...
            if (failedReboots.length) {
                failedReboots.forEach(function (fr) {
                    p('- Failed reboot of%s server %s: platform %s -> %s',
                            (cSUuids.indexOf(fr.server_uuid) !== -1 ?
                                ' core' : ''),
                            fr.server_uuid,
                            fr.current_platform,
                            fr.boot_platform);
...
RebootPlan.prototype.pause = function pause(callback) {
    var self = this;
    self.getCurrentPlan(function (err, plan) {
        ...
        if (plan.state === 'paused' || plan.state === 'created') {
        ...
        self.sdcadm.cnapi.put(planUri, {
            ...
            p('Plan execution has been paused');
            callback();
        });
    });
};

RebootPlan.prototype.cancel = function cancel(uuid, callback) {
    var self = this;
    assert.string(uuid, 'uuid');
    self.getPlan(uuid, function (err, plan) {
        if (err) {
        ...
        if (plan.state === 'running') {
            p('Plan is running. Please, pause it before cancelling');
            callback();
            return;
        }
...
RebootPlan.prototype.watch = function watch(callback) {
    ...
    self.getCurrentPlan(function (err, plan) {
        ...
        if (plan.state === 'created' || plan.state === 'paused') {
        ...
        process.on('SIGINT', function rebootPlanInfo() {
            p('paused watching reboot plan %s', plan.uuid);
            p('**Note: The reboot plan is still running! ' +
                    'Use `sdcadm reboot-plan pause` to pause it.**');
            process.exit(0);
        });
...
RebootPlanCLI.prototype.do_create.options = [
    ...
    },
    {
        names: ['name'],
        type: 'string',
        help: 'Optional descriptive name for the reboot plan'
    },
    {
...
RebootPlanCLI.prototype.do_run = function do_run(subcmd, opts, args, cb) {
    ...
    if (!args.length) {
        return cb(new errors.UsageError(
            'too few args, plan uuid is required'));
    }
...
RebootPlanCLI.prototype.do_status = function do_status(subcmd, opts, args, cb) {
    ...
    self.rebootPlan.status(args[0], cb);
};
...
RebootPlanCLI.prototype.do_pause = function do_pause(subcmd, opts, args, cb) {
    var self = this;
    if (opts.help) {
        self.do_help('help', {}, [subcmd], cb);
        return;
    }
    self.rebootPlan.pause(cb);
};

RebootPlanCLI.prototype.do_pause.help = (
...
RebootPlanCLI.prototype.do_pause.options = [
    {
        names: ['help', 'h'],
        type: 'bool',
        help: 'Show this help.'
...
RebootPlanCLI.prototype.do_cancel = function do_cancel(subcmd, opts, args, cb) {
    ...
    self.rebootPlan.cancel(args[0], cb);
};
...
// Run just a single step of the reboot plan, and pause there until operator
