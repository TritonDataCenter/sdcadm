...
var util = require('util'),
    format = util.format;
function d(arg) {
    p(util.inspect(arg, false, 8, true));
}
    ...
    self.getCurrentPlan(function (err, plan) {
        ...
        self.sdcadm.cnapi.put(planUri, {
            ...
            svcadm.svcadmRestart({
                fmri: '/smartdc/sdcadm-reboot-plan:default',
                log: self.log
            }, function (svcadmErr) {
                if (svcadmErr) {
                    callback(svcadmErr);
    ...
    vasync.pipeline({
        ...
        funcs: [
            function getRebootJob(arg, next) {
                self.sdcadm.wfapi.get(arg.job_uuid, function (err, job) {
                    if (err) {
            ...
            function pollJobFinished(arg, next) {
                function _pollJobFinished(cb) {
                    ...
                    var u = format('/reboot-plans/%s/reboots/%s',
                            arg.reboot_plan_uuid, arg.reboot_uuid);
                ...
                _pollJobFinished(function (err, reb) {
                    if (err) {
                        next(err);
                        return;
                    }

                    arg.started_at = reb.started_at;
                    arg.finished_at = reb.finished_at;
                    next();
                });
            },
...
RebootPlan.prototype.pollPlan = function pollPlan(opts, callback) {
    ...
    self.sdcadm.get('/reboot-plans/' + plan.uuid, function (cnapiErr, aPlan) {
        if (cnapiErr) {
            callback(cnapiErr);
            return;
        }
        ...
        self.getCoreServers(function (cSErr, cSUuids) {
            ...
            function doPollPlan() {
                self.pollPlan({
                    ...
                    if (aPlan.state !== 'running') {
                        self.progress('Plan ' + aPlan.state);
                        bar.end();
                        callback();
                        return;
                    }
...
RebootPlanCLI.prototype.do_run.help = (
    'Execute/continue the reboot plan.\n' +
    '\n' +
    '{{options}}'
);
...
RebootPlanCLI.prototype.do_watch.help = (
    'Watch (and wait for) the current running reboot plan.\n' +
    '\n' +
    '{{options}}'
);
...
RebootPlanCLI.prototype.do_stop.help = (
    'Stop execution of the current (running) reboot plan.\n' +
    '\n' +
    '{{options}}'
);
