...
// TODO: opts.singleStep should be saved into moray in order to make possible
// to get if after a reboot.
...
var options = [
    ...
    },
    {
        name: ['single-step', 's'],
        type: 'bool',
        help: 'Reboot a single server of the reboot plan and exit.'
    }
];
...
RebootPlanRunner.prototype.run = function run(opts, callback) {
    ...
    assert.optionalBool(opts.singleStep, 'opts.singleStep');
    ...
    self.sdcadm.cnapi.get(planQs, function getPlansCb(getPlansErr, plans) {
        ...
        var steps = opts.singleStep ? [reboots[0]] : reboots;
        var successAction = (!opts.singleStep || reboots.length === 1) ?
                                'finish' : 'stop';
