...
RebootPlanRunner.prototype._getRunningPlanOrLatest =
function _getRunningPlanOrLatest(callback) {
    var self = this;
    var planQs = '/reboot-plans?include_reboots=true&state=pending';
    self.sdcadm.cnapi.get(planQs, function (getPlansErr, plans) {
        if (getPlansErr) {
            return callback(new errors.SDCClientError(getPlansErr, 'cnapi'));
        }

        if (plans.length) {
            return callback(null, plans[0]);
        }

        planQs = '/reboot-plans?include_reboots=true&limit=1';
        self.sdcadm.cnapi.get(planQs, function (gPlansErr, plans2) {
            if (gPlansErr) {
                return callback(new errors.SDCClientError(gPlansErr, 'cnapi'));
            }

            return callback(null, plans2[0]);
        });
...
RebootPlanRunner.prototype.run = function run(callback) {
    ...
    self._getRunningPlanOrLatest(function getPlansCb(getPlansErr, plan) {
        if (getPlansErr) {
            return callback(new errors.SDCClientError(getPlansErr, 'cnapi'));
        }

        if (!plan) {
            self.progress('There are no pending reboot plans.');
            return callback(false);
        }

        self.plan = plan;

        if (self.plan.state === 'complete') {
            return callback(false);
        }

        var runningReboots = self.plan.reboots.filter(function (r) {
            return (r.state === 'initiated' || r.state === 'rebooting');
        });

        if (self.plan.state === 'created') {
            self.progress('Reboot plan is not running');
            return callback(false);
        }

        if ((self.plan.state === 'canceled' || self.plan.state === 'paused') &&
                !runningReboots.length) {
            self.progress('Reboot plan is not running');
            return callback(false);
        }

        var reboots = self.plan.reboots.filter(function (r) {
            return (r.state !== 'canceled' && r.state !== 'failed' &&
                    r.state !== 'complete');
        });

        if (!reboots.length) {
            self.progress('Reboot plan is already completed.');
        ...
        function doneCb(err) {
            ...
            if (successAction === 'finish') {
                self.progress('Reboot plan completed.');
            } else {
                self.progress('Reboot plan\'s step completed.');
            }
            return self.finishPlan(successAction, callback);
        }
        ...
        // Given we want to be able to pause the execution of the plan during
        ...
        // well or, in case of failure, pause the process as soon as possible.
...
RebootPlanRunner.prototype.doReboot = function doReboot(reboot, callback) {
    ...
    assert.func(callback, 'callback');
    var self = this;
    // Check if the plan has been canceled or paused before we go ahead with
    // the reboot. (The plan can be paused or canceled during the execution
    ...
    self.sdcadm.cnapi.get(planQs, function (getPlanErr, plan) {
        ...
        vasync.pipeline({
            arg: {reboot: reboot},
            funcs: [
                function getServer(ctx, next) {
                    var id = ctx.reboot.server_uuid;
                    self.sdcadm.cnapi.getServer(id, function (err, srv) {
                        if (err) {
                            return next(new VError(err,
                                    'Error getting server %s from CNAPI', id));
                        }
                        ctx.server = srv;
                        return next();
                    });
                ...
                function checkHeadnodeSvcs(ctx, next) {
                    if (!ctx.server.headnode || !ctx.reboot.job_uuid) {
                        return next();
                    }
                    self.log.trace({
                        server: ctx.reboot.server_uuid
                    }, 'Waiting for headnode core SDC services');
                    self.waitForSvcs(ctx.reboot, function (err, svcs) {
                        if (err) {
                            return next(new VError(err,
                                    'Cannot verify headnode services health'));
                        }
                ...
                function prepareManatee(ctx, next) {
                    ...
                    self.sdcadm.listInsts({
                        ...
                        shared.getShardState({
                            ...
                            if (err2) {
                                return next(new VError(err2,
                                            'Cannot get manatee shard state'));
                            }
                            ...
                            // Manatee should be already frozen if the job ran
                            ...
                            common.manateeFreeze({
                                ...
                                if (err3) {
                                    return next(new VError(err3,
                                                'Cannot freeze manatee shard'));
                                }
                                return next();
                            });
                        });
                ...
                function getRebootJob(ctx, next) {
                    self.sdcadm.wfapi.getJob(ctx.reboot.job_uuid,
                        ...
                        if (err) {
                            return next(new VError(err,
                                        'Cannot retrieve reboot job'));
                        }

                        ctx.job_finished = (ctx.reboot.state !== 'created' &&
                                ctx.reboot.state !== 'initiated') ? true :
                ...
                function pollRebootJob(ctx, next) {
                    ...
                    self.waitForJob(ctx.reboot.job_uuid, function (jErr, job) {
                        if (jErr) {
                            return next(new VError(jErr,
                                        'Error polling reboot job'));
                        }
                ...
                function checkCoreSvcs(ctx, next) {
                    ...
                    if (!ctx.job_execution ||
                        ...
                        return next(new errors.InternalError({message: format(
                            'Reboot job failed for server %s',
                            ctx.reboot.server_uuid)}));
                    }
                    self.log.trace({
                        server: ctx.reboot.server_uuid
                ...
                function updateReboot(ctx, next) {
                    self.log.trace({
                        server: ctx.reboot.server_uuid
                    }, 'Updating reboot record (complete)');
                    var rebootUrl = format('/reboot-plans/%s/reboots/%s',
                            self.plan.uuid, ctx.reboot.uuid);
                    self.sdcadm.cnapi.put(rebootUrl, {
                        state: 'complete',
                        events: [ {
                            type: 'server_reboot_complete',
                            time: new Date().toISOString()
                        }]
                    }, function (err) {
                        if (err) {
                            return next(new VError(err,
                                        'Error updating reboot in CNAPI'));
                        }
                        return next(null);
                    });
                }
            ...
            if (rebootErr) {
                self.log.error({
                    ...
                    }, 'Updating reboot record (failed)');
                // Attempt to "fail" the reboot before buble up the error:
                var rebootUrl = format('/reboot-plans/%s/reboots/%s',
                        self.plan.uuid, reboot.uuid);
                self.sdcadm.cnapi.put(rebootUrl, {
                    state: 'failed',
                    events: [ {
                        type: 'server_reboot_failed',
                        time: new Date().toISOString()
                    }],
                    error: rebootErr
                }, function (cancelErr) {
