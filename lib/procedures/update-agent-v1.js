...
UpdateAgentV1.prototype.execute = function uaExecute(opts, callback) {
    ...
    // for all the agents
    let bar;
    // And need to keep track of the bar size when resizing for multiple
    // parallel updates
    let multiBarSize;
    // Services we cannot update in parallel b/c are needed for other updates:
    const serialUpSvcs = ['agents_core', 'cn-agent', 'config-agent'];
            ...
            function updateAgentOnServers(_, next) {
                if (process.stderr.isTTY) {
                    if (change.type === 'create-instances' ||
                        serialUpSvcs.indexOf(change.service.name) !== -1) {
                        completed = 0;
                        bar = new ProgressBar({
                        ...
                        if (!bar) {
                            bar = new ProgressBar({
                                size: change.insts.length,
                                bytes: false,
                                filename: 'Updating agents'
                            });
                            multiBarSize = change.insts.length;
                        } else {
                            multiBarSize += change.insts.length;
                            bar.resize(multiBarSize);
                        }
                        bar.log(format('Updating agent %s',
                                change.service.name));
                    }
                }
                ...
                queue.on('end', function done() {
                    if (bar && (change.type === 'create-instances' ||
                        serialUpSvcs.indexOf(change.service.name) !== -1)) {
                        bar.end();
                        bar = null;
                    }
    ...
    checklist(function (err) {
        ...
        let i;
        for (i = 0; i < self.changes.length; i += 1) {
            if (serialUpSvcs.indexOf(self.changes[i].service.name) !== -1) {
                ctx.serialChanges.push(self.changes[i]);
            } else {
                ctx.parallelChanges.push(self.changes[i]);
            }
        }

        vasync.pipeline({arg: ctx, funcs: [
            function execSerialChanges(arg, serialCb) {
                let serialChgs = arg.serialChanges;
                if (!serialChgs.length) {
                    serialCb();
                    return;
                }
            ...
            function execParallelChanges(arg, paraCb) {
                let paraChgs = arg.parallelChanges;
                if (!paraChgs.length) {
                    paraCb();
                    return;
                }
