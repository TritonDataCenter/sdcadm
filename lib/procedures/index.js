...
var EnsureMantaDeploymentGzLinksProcedure =
...
function coordinatePlan(opts, cb) {
    ...
    vasync.pipeline({funcs: [
        ...
        /**
         * Add the procedure for downloading images
         * (from updates.tritondatacenter.com), if necessary.
         */
        ...
        function updateSimpleServices(_, next) {
            ...
            if (handle.length) {
                ...
                // Special case: If we are updating the manta deployment zone
                // then be sure to update the GZ symlinks it wants (e.g.
                // 'manta-adm').
                for (var h = 0; h < handle.length; h++) {
                    if (handle[h].service.name === 'manta') {
                        procs.push(new EnsureMantaDeploymentGzLinksProcedure());
                        break;
                    }
                }
            }
