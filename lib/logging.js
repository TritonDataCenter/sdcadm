 ...
 * - We log level FATAL to stderr on the assumption that these messages
 *   may be emitted synchronously on the way to a panic, and that the
 *   usability issues with raw JSON output are outweighed by the need
 *   to report actionable information in bug reports.
...
function createLogger(opts) {
    ...
    assert.optionalString(opts.component, 'opts.component');
    ...
    // Log streams:
    // 1. Stream to stderr at FATAL, at TRACE if `verbose`.
    var logStreams = [
        {
            stream: process.stderr,
            level: (opts.verbose ? 'trace' : 'fatal')
        }
    ];
    ...
    var slug = '';
    if (opts.component) {
        slug = '-' + opts.component.replace(/[^a-zA-Z0-9]+/g, '-');
    }
    var logFile = sprintf('%s/%s-%06d%s.log',
        LOG_DIR, Date.now(0), process.pid, slug);
    if (opts.logToFile) {
        logStreams.push({
            level: 'trace',
            path: logFile
