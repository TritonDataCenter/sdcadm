...
function ensureMantaDeploymentSvcAndInst(cli, opts, wantMantav2, cb) {
    ...
    if (wantMantav2) {
        cli.ui.info([
            ...
            'This will setup for a new Manta v2 deployment, or update an existing',
            'mantav2 deployment zone.',
            '',
            'This creates a zone on the headnode, called the manta-deployment zone,',
            'which provides the tooling for deploying and maintaining a Manta installation.',
            'After this step is complete, follow the Manta Operator Guide to deploy Manta:',
            '    https://github.com/TritonDataCenter/manta/blob/master/docs/operator-guide/deployment.md#deploying-manta'
            /* eslint-enable */
        ].join('\n'));
    } else {
        cli.ui.info([
            /* eslint-disable max-len */
            'This will setup for a new Manta v1 deployment, or update an existing',
            'mantav1 deployment zone.',
            '',
            'Note that mantav1 is no longer the latest version of Manta.',
            'See the following for information on mantav1 vs mantav2:',
            '    https://github.com/TritonDataCenter/manta/blob/master/docs/mantav2.md',
            '',
            'This creates a zone on the headnode, called the manta-deployment zone,',
            'which provides the tooling for deploying and maintaining a Manta installation.',
            'After this step is complete, follow the Manta Operator Guide to deploy Manta:',
            '    https://github.com/TritonDataCenter/manta/blob/mantav1/docs/operator-guide.md'
            /* eslint-enable */
        ].join('\n'));
    }
...
function startMantav2Migration(cli, opts, cb) {
    ...
    vasync.pipeline({funcs: [
        function confirmWantMigration(_, next) {
            ...
            ui.info('jobs and snaplinks. Read the following for more details:');
            ui.info('    https://github.com/TritonDataCenter/manta/blob/master/docs/mantav2.md');
            ui.info('* * *');
            /* eslint-enable */
            ...
            common.promptConfirm({
                msg: 'Enter "mantav2" to confirm you would like to migrate: ',
                confirmationStr: 'mantav2'
            }, function (confirmed) {
        ...
        function followupInstructions(_, next) {
            ui.info([
                '',
                'To complete the mantav2 migration follow these instructions:',
                '    https://github.com/TritonDataCenter/manta/tree/master/docs/operator-guide/mantav2-migration.md'
            ].join('\n'));
            next();
        }
