...
CLI.prototype.do_instances = function do_instances(subcmd, opts, args,
    ...
    self.sdcadm.getInstances({}, function (err, insts) {
        ...
        if (opts.group_by_image) {
            ...
            rows = Object.keys(rowFromTypeSvcImage).map(function (key) {
                var row = rowFromTypeSvcImage[key]
...
CLI.prototype.do_update = function do_update(subcmd, opts, args, cb) {
    ...
    vasync.pipeline({funcs: [
        /**
         * Also see 'sdcadm update' section in design.md.
         *
         *      sdcadm update <svc> ...
        ...
        function getLock(_, next) {
            self.sdcadm.acquireLock({logCb: console.log},
        ...
        function getSpecFromArgs(_, next) {
            ...
            for (var i = 0; i < args.length; i++) {
                ...
                var change = {};
                if (inst) change.instance = inst;
                if (svc) change.service = svc;
                if (img) change.image = img;
                if (ver) change.version = ver;
                changes.push(change)
            }
...
if (require.main === module) {
    ...
    cli.main(process.argv, function (err, subcmd) {
        ...
        if (err) {
            ...
            showErr && cli.log && cli.log.error(
                {err: err, subcmd: subcmd, exitStatus: exitStatus}, 'cli exit');
