    ...
    function displayResults(err, statuses) {
        ...
        var rows = statuses.map(function (status) {
            return {
                ...
                instance_alias: status.instance.alias,
                instance_uuid:  status.instance.uuid
            };
        });

        var sortAttr = ['service', 'instance_alias', 'instance_uuid'];
        common.sortArrayOfObjects(rows, sortAttr);

        if (opts.json) {
            console.log(JSON.stringify(rows, null, 4));
        } else {
            var columns = ['service', 'instance_alias', 'instance_uuid',
                           'healthy'];
...
CLI.prototype.do_check_health.help = (
    'Check that sdc services or instances are up.\n'
    + '\n'
    + 'Usage:\n'
    + '     {{name}} check-health [<options>]\n'
    + '     {{name}} check-health [<options>] <svc> ...\n'
    + '     {{name}} check-health [<options>] <inst> ...\n'
    + '\n'
    + '{{options}}'
);
...
if (require.main === module) {
    ...
    cli.main(process.argv, function (err, subcmd) {
        if (err) {
            ...
            if (showErr && cli.opts.verbose && cli.log) {
                cli.log.error(
