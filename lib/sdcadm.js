 ...
 *                      updates.tritondatacenter.com. Until they are
 *                      `image === null`.)
                ...
                funcs: [
                    ...
                    function getDockerLoggerImage(arg, nextFun) {
                        ...
                        self.imgapi.getImage(imgUuid, function (imgErr, img) {
                            if (imgErr) {
                                if (imgErr.restCode === 'ResourceNotFound') {
                                    nextFun();
                                } else {
                                    nextFun(new errors.SDCClientError(
                                        imgErr, 'imgapi'));
...
/**
 * Return an array of suitable image names for the given service. Typically
 * this is a static set, and typically is a single name. However:
 *
 * - The image name changes, due to mantav2, mean two names are appropriate
 *   for the images shared with manta, e.g. `mantav2-nameservice` and
 *   `manta-nameservice` for binder.
 * - The manta deployment service needs to select the appropriate image names
 *   depending on the current major manta version (mantav), if any.
 *
 ...
 * @param cb {Function} `function (err, imgNames)`
 *      If there are no known valid image names for this service (e.g. this is
 *      some errant service name in SAPI), then this returns `cb(null, null)`.
 */
SdcAdm.prototype.imgNamesFromSvcName =
function imgNamesFromSvcName(svcName, cb) {
 ...
 *      - insts {Array} Required. Current DC instances as from `listInsts()`.
 *      - progress {Function} Required.
 * @param cb {Function} `function (err, imgs)`
 *      Typically this returns an array of image manifests. However, if
 *      the service is unknown (e.g. some lingering service long since obsolete)
 *      then this will emit a warning and return `cb(null, [])`.
 */
SdcAdm.prototype.getCandidateImages = function getCandidateImages(opts, cb) {
    ...
    vasync.pipeline({funcs: [
        function getValidImgNames(_, next) {
            self.imgNamesFromSvcName(opts.service.name,
                function (err, imgNames) {
                    ...
                    validImgNames = imgNames;
                    self.log.debug({validImgNames: validImgNames},
                        'getCandidateImages: getValidImgNames');

                    if (!validImgNames) {
                        var msg = format(
                            'warning: do not know valid image names for ' +
                                'service "%s"',
                            opts.service.name);
                        opts.progress(msg);
                        imgs = [];
        ...
        function getCandidates(_, next) {
            ...
            let marker = (currImgs.length > 0 ?
                currImgs[0].published_at : undefined);
            self.log.debug({marker: marker, validImgNames: validImgNames},
                'getCandidateImages: getCandidates');

            var allCandidates = [];
            vasync.forEachPipeline({
                inputs: validImgNames,
                func: function listOneName(name, nextName) {
                    self.updates.listImages({
                        name: name,
                        marker: marker
                    }, function (uErr, candidates) {
                        if (uErr) {
                            nextName(uErr);
                            return;
                        }

                        // Sanity check that IMGAPI properly returns images with
                        // the given name (e.g. TOOLS-745).
                ...
                if (currImgs.length > 0) {
                    ...
                    imgs = [currImgs[0]].concat(allCandidates);
                } else {
                    imgs = allCandidates;
                }
                next();
...
SdcAdm.prototype.acquireLock = function acquireLock(opts, cb) {
    ...
    qlocker.lock(self._lockPath, function (lockErr, unlock) {
        ...
        function normalizeChanges(_, next) {
            ...
            vasync.forEachParallel({inputs: changes, func:
                function resolveChange(ch, nextChange) {
                    ...
                    if (ch.image) {
                        self.imgNamesFromSvcName(ch.service.name,
                                                 function (err, imgNames) {
                            if (err) {
                                nextChange(err);
                                return;
                            }
                            self.getImage({uuid: ch.image}, function (e, img) {
                                if (e) {
                                    nextChange(new errors.UpdateError(e,
                                        format('unknown image "%s" from %s',
                                            ch.image, changeRepr)));
                                    return;
                                }
                                // Make sure we're not trying to update a
                                // service to a different service image:
                                if (imgNames.indexOf(img.name) === -1) {
                                    nextChange(new errors.UpdateError(format(
                                        'image %s (%s@%s) is invalid for ' +
                                        'service %s, valid images must have ' +
                                        'name "%s" (update change: %s)',
                                        img.uuid,
                                        img.name,
                                        img.version,
                                        ch.service.name,
                        ...
                        self.imgNamesFromSvcName(ch.service.name,
                            ...
                            self.updates.listImages({
                                ...
                                // Limit our search by 'version' to those with
                                // appropriate image names.
                                imgs = imgs.filter(function (img) {
                                    return imgNames.indexOf(img.name) !== -1;
                                });
                                if (!imgs.length) {
                                    nextChange(new errors.UpdateError(format(
                                        'no image with version "%s" and ' +
                                            'name "%s" was found (update ' +
                                            'change: %s)',
                                        ch.version,
                                        imgNames.join('" or "'),
                                        changeRepr)));
                                ...
                                common.sortArrayOfObjects(imgs,
                                    ['published_at']);
                                ch.images = imgs;
                                delete ch.version;
                                nextChange();
            ...
            function checkMinImageBuildDate(ins) {
                ...
                if (!ins.version) {
                    var msg = format('Unknown image ' +
                        ...
                        '\n\n    sdc-imgadm ' +
                        'import %s -S https://updates.tritondatacenter.com?' +
