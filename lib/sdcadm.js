    ...
    vasync.pipeline({funcs: [
        ...
        function getServers(_, next) {
            self.cnapi.listServers(function (err, servers_) {
                ...
                for (var i = 0; i < servers.length; i++) {
                    serverFromUuid[servers[i].uuid] = servers[i];
                }
                next(err);
            });
        },
        ...
        function checkForConflictingChanges(_, next) {
            ...
            for (var i = 0; i < changes.length; i++) {
                ...
                var typeTarg = ch.type.split('-')[1];
                if (typeTarg === 'service') {
                    var svc = ch.service.name;
            ...
            for (var i = 0; i < changes.length; i++) {
                ...
                if (ch.type === 'update-service') {
                    for (var j = 0; j < insts.length; j++) {
                        var inst = insts[j];
                        if (inst.service === ch.service.name) {
                            server = serverFromUuid[inst.server];
                    ...
                    server = serverFromUuid[ch.server];
                    TODO
                }
            }
            next();
