...
SdcAdm.prototype.dcMaintStart = function dcMaintStart(opts, cb) {
    ...
    vasync.pipeline({funcs: [
        ...
        function setDCMaintenance(_, next) {
            ...
            progress('Putting data center into maintenance mode');
            startTime = new Date();
...
SdcAdm.prototype.checkHealth = function checkHealth(opts, cb) {
    ...
    // In case ur connection fails due to any cause, we want to take an
    // alternate path and just check local service instances.
    var urConnectionFailed;
