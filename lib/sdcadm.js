 ...
 * XXX
 */
SdcAdm.prototype.selfUpdate = function selfUpdate(cb) {
    var self = this;
    assert.func(cb, 'cb');
    var log = self.log;
    ...
    vasync.pipeline({funcs: [
        function findLatestSdcAdm(_, next) {
            // TODO: imgadm.js "getLatest(filter, cb)" function
            var filters = {
                name: 'sdcadm'
            };
            self.updates.listImages(filters, function (err, pkgs) {
                if (err) {
