...
function do_dc_maint(subcmd, opts, args, cb) {
    ...
    } else if (opts.cloudapi_only && opts.docker_only) {
        cb(new errors.UsageError(
                    'cannot use --cloudapi-only and --docker-only'));
    } else if (opts.start) {
...
do_dc_maint.help = (
    ...
    ' DC maint\n' +
    '     {{name}} dc-maint --start [--cloudapi-only | --docker-only] # start' +
    ' DC maint only for CloudAPI or Docker services. Mutally exclusive.\n' +
    '\n' +
    '{{options}}'
);
