...
function do_update_other(subcmd, opts, args, cb) {
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function ensureCloudapiSvcIsCore(ctx, next) {
            ...
            if (!cloudapiSvc.params.tags.smartdc_type) {
                progress('Set tag smartdc_type=="core" for cloudapi service');
                self.sdcadm.sapi.updateService(cloudapiSvc.uuid, {
                    params: {
                        tags: {
        ...
        function ensureCloudapiInstsAreCore(_, next) {
            self.sdcadm.vmapi.listVms({
                'tag.smartdc_role': 'cloudapi',
                state: 'running',
                owner_uuid: self.sdcadm.config.ufds_admin_uuid
            }, function (vmsErr, cloudapiVms) {
                if (vmsErr) {
                    next(vmsErr);
                    return;
                }

                var toUpdate = cloudapiVms.filter(function (vm) {
                    return typeof vm.tags.smartdc_type === 'undefined';
                });
                if (!toUpdate.length) {
                    next();
                    return;
                ...
                progress('Setting tag smartdc_type==core on %d cloudapi: %s',
                    toUpdate.length,
                    toUpdate.map(
                        function (vm) {
                            return vm.alias;
                ...
                vasync.forEachPipeline({
                    inputs: toUpdate,
                    func: function setCloudapiTags(vm, nextVm) {
                        // TODO: add vmapi.updateVmAndWait and use that
                        var vmTags = vm.tags;
                        vmTags.smartdc_type = 'core';
                        self.sdcadm.vmapi.updateVm({
                            uuid: vm.uuid,
                            payload: {
                                tags: vmTags
                            }
                        }, function (updateErr, jobInfo) {
                            if (updateErr) {
                                nextVm(new errors.SDCClientError(
                                    updateErr, 'cloudapi'));
                                return;
                            }
                            progress('Adding tag on vm %s (%s). Note: *not* ' +
